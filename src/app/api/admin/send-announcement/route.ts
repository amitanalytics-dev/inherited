...
const KLAVIYO_LIST_ID = 'SvpFav' // Newsletter list
...
function buildHtml(body: string, preheader: string): string {
  ...
  const paragraphs = body
    ...
    .map((b) => b.trim())
    .filter(Boolean)
    .map((b) => `<p style="margin:0 0 16px;font-family:Arial,Helvetica,sans-serif;font-size:15px;color:#2D2926;line-height:1.7">${b.replace(/\n/g, '<br>')}</p>`)
    .join('\n')
...
function buildSendAt(dateStr: string): string {
  // dateStr is YYYY-MM-DD; always send at 17:00 UTC (= 6pm BST in summer)
...
export async function POST(req: NextRequest) {
  ...
  const validThrottles = [10, 11, 13, 14, 17, 20, 25, 33, 50]
  const throttle = validThrottles.includes(throttlePercent) ? throttlePercent : 10

  const sendAt = scheduleDate ? buildSendAt(scheduleDate) : (() => {
    const now = new Date()
    if (now.getUTCHours() >= 17) now.setUTCDate(now.getUTCDate() + 1)
    return `${now.toISOString().slice(0, 10)}T17:00:00+00:00`
  })()
  ...
  try {
    ...
    const tmplRes = await kFetch('/email-templates/', 'POST', {
      data: {
        ...
        attributes: {
          ...
          editor_type: 'CODE',
          html: buildHtml(htmlBody, preheader),
    ...
    // 2. Create campaign with throttled send strategy
    ...
    // 3. Update campaign message: set subject, from details, and assign template
    ...
    return NextResponse.json({ success: true, campaignId, templateId, sendAt })
  } catch (err: unknown) {
    const msg = err instanceof Error ? err.message : 'Unknown error'
    console.error('[send-announcement]', msg)
