...
export default function ProductCard({ product, className }: ProductCardProps) {
  ...
  const avgRating = shopifyRating ?? localAvg
  const reviewCount = shopifyCount ?? productReviews.length
      ...
      <div className="space-y-1.5">
        ...
        {avgRating !== null && (
          <div className="flex items-center gap-1 mt-1">
            ...
            <span className="font-body text-[11px] text-brand-muted ml-0.5">({reviewCount})</span>
          </div>
        )}
      </div>
    </Link>
